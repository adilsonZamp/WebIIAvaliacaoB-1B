...
export default function CriarContaCorrenteModal({
	...
	const handleChange = (
		...
		if (name === 'numeroAgencia' && !Number.isInteger(Number(value))) {
			return
		} else if (name === 'digito'  && !Number.isInteger(Number(value))) {
			return
		}
	...
	const handleSubmit = async (e: FormEvent) => {
		...
		const { numeroAgencia, digito, saldo, clienteId } = form;
		if (!numeroAgencia || !numeroAgencia || !saldo || !clienteId) {
			setError("Preencha todos os campos obrigatórios.");
			return;
		}

		console.log(numeroAgencia + " " + digito + " " + saldo + " " + clienteId);
		...
		try {
			...
			const response = await Client.post('criarContaCorrente', {
