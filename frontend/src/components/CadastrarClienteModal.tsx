...
export default function CadastrarClienteModal({
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    const { nomeCompleto, email, senha, cpf, cidade, estado, rua, numero } = form;
    if (!nomeCompleto || !email || !senha || !cpf || !cidade || !estado || !rua || !numero) {
    ...
    try {
      ...
      const response = await fetch("/api/clientes", {
        ...
        body: JSON.stringify({
          ...
          senha,
          cpf: cpf.replace(/\D/g, ""),
          endereco: { cidade, estado, rua, numero },
        }),
      });
